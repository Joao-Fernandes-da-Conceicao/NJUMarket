...
public class CommoditySearchService {
    ...
    /**
     * 主排序：相关度 / 价格 / 时间；最后追加 {@code commodityId asc} 作为全局稳定 tie-breaker，
     * 避免同分或同价时顺序在 refresh/分页间抖动。
     */
    private Sort resolveSort(String sortBy) {
        Sort primary;
        if (!StringUtils.hasText(sortBy)) {
            primary = Sort.by(Sort.Order.desc("_score"));
        } else {
            primary = switch (sortBy) {
        ...
        return primary.and(Sort.by(Sort.Order.asc("commodityId")));
    }
