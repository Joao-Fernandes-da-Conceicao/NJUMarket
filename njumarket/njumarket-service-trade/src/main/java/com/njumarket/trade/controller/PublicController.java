...
public class PublicController {
    ...
    @Operation(summary = "搜索商品", description = "根据关键词搜索商品，支持价格、位置、分类等筛选条件；公开列表最多 20 页×10 条（共 200 条）内排序与分页")
    ...
    public Result searchCommodities(
            ...
            @Parameter(description = "页码（最大 20）", example = "1") @RequestParam(defaultValue = "1") Integer page,
            @Parameter(description = "每页数量（最大 10）", example = "10") @RequestParam(defaultValue = "10") Integer size,
    ...
    @Operation(summary = "按分类获取商品", description = "根据分类获取商品列表；公开列表最多 20 页×10 条")
    ...
    @Operation(summary = "获取指定卖家的商品列表", description = "获取指定卖家的商品列表（公开可见，排除草稿），支持状态筛选；访客视角最多 20 页×10 条")
    ...
    public Result getSellerCommodities(
            ...
            @Parameter(description = "页码（访客最大 20）", example = "1") @RequestParam(defaultValue = "1") Integer page,
            @Parameter(description = "每页数量（访客最大 10）", example = "10") @RequestParam(defaultValue = "10") Integer size) {
        // 公共接口，user 参数为 null
