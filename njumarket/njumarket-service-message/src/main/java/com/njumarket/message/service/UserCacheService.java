...
public class UserCacheService {
    ...
    private static final TypeReference<List<UserProfileInternalDTO>> PROFILE_LIST_TYPE =
            new TypeReference<>() {};
    private static final TypeReference<List<UserInternalDTO>> USER_LIST_TYPE =
            new TypeReference<>() {};
    ...
    /**
     * 批量获取用户基础信息（Redis → Feign 回退）
     *
     * @return Map&lt;userId, UserInternalDTO&gt;
     */
    public Map<String, UserInternalDTO> getUsersByIds(Collection<String> userIds) {
        if (userIds == null || userIds.isEmpty()) return Collections.emptyMap();

        Map<String, UserInternalDTO> result = new LinkedHashMap<>();
        List<String> misses = new ArrayList<>();

        for (String uid : userIds) {
            if (!StringUtils.hasText(uid)) continue;
            UserInternalDTO cached = cacheUtil.get(
                    RedisConstants.CACHE_USER_INFO_KEY + uid, UserInternalDTO.class);
            if (cached != null) {
                result.put(uid, cached);
            } else {
                misses.add(uid);
        ...
        if (!misses.isEmpty()) {
            try {
                Result feignResult = authClient.getUsersByIds(misses);
                if (feignResult != null && feignResult.getSuccess() && feignResult.getData() != null) {
                    List<UserInternalDTO> users =
                            objectMapper.convertValue(feignResult.getData(), USER_LIST_TYPE);
                    for (UserInternalDTO u : users) {
                        cacheUtil.set(RedisConstants.CACHE_USER_INFO_KEY + u.getUserId(),
                                u, RedisConstants.CACHE_USER_INFO_TTL * 60L);
                        result.put(u.getUserId(), u);
                    }
                }
            } catch (Exception e) {
                log.warn("批量 Feign 获取用户信息失败: misses={}, error={}", misses, e.getMessage());
            }
        }
