...
public class WebSocketConfig implements WebSocketMessageBrokerConfigurer {
    ...
    public void configureMessageBroker(MessageBrokerRegistry config) {
        ...
        // 客户端订阅 /user/queue/message 来接收消息
        ...
        // 设置用户目标前缀
        // 服务器发送消息到 /user/{userId}/queue/message
    ...
    static class WebSocketHandshakeInterceptor implements HandshakeInterceptor {
        ...
        public boolean beforeHandshake(ServerHttpRequest request, ServerHttpResponse response,
            ...
            if (userId != null && !userId.trim().isEmpty()) {
                log.info("WebSocket握手: 提取userId={}", userId);
                
                // ✅ 将userId存储到attributes中，供ChannelInterceptor使用
                attributes.put("userId", userId);
                
                log.info("WebSocket握手: 已存储userId到attributes, userId={}", userId);
                return true;
            } else {
                log.warn("WebSocket握手: 未找到X-User-Id请求头，拒绝连接");
                return false; // 拒绝连接
            }
        }
        ...
        public void afterHandshake(ServerHttpRequest request, ServerHttpResponse response,
            ...
            if (exception != null) {
                log.error("WebSocket握手失败: error={}", exception.getMessage(), exception);
            } else {
                String userId = request.getHeaders().getFirst("X-User-Id");
        ...
        public Message<?> preSend(Message<?> message, MessageChannel channel) {
            ...
            if (accessor != null && StompCommand.CONNECT.equals(accessor.getCommand())) {
                ...
                if (sessionAttributes != null) {
                    ...
                    if (userId != null && !userId.trim().isEmpty()) {
                        log.info("WebSocket CONNECT: 设置Principal, userId={}", userId);
                        ...
                        log.info("WebSocket CONNECT: Principal已设置, name={}", userId);
                    } else {
                        log.warn("WebSocket CONNECT: 未找到userId in sessionAttributes");
                    }
                } else {
                    log.warn("WebSocket CONNECT: sessionAttributes为null");
                }
            }
